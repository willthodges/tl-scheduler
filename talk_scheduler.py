    ...
    for _ in range(N_POP):
        ...
        for i in range(N_SESSIONS):
            if i != N_SESSIONS-1:
                ...
                session_new = random.sample(teacher_list, N_ROOMS*3)
            else:
                session_new = random.sample(teacher_list, leftover*3)
...
def selection(talk_pop, panel_pop, scores_list, N_SELECTION):
    ...
    selection_i = randint(len(talk_pop))
    for i in randint(0, len(talk_pop), N_SELECTION-1):
